...
(async () => {
  ...
  const selectedSongHref = await iframe.evaluate(e => {
    ...
    const idx = songList.findIndex(v => v.childNodes[1].innerText.replace(/\s/g, '') === '樱花樱花想见你');
  ...
  await unfoldButton.click({ delay: 0 });
  // 获取歌词
  ...
  // 写入文件
  let writerStream = fs.createWriteStream('樱花樱花想见你.txt');
